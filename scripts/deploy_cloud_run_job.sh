...
BASE_ENV_VARS="${BASE_ENV_VARS:-GOOGLE_CLOUD_PROJECT=${PROJECT}||BQ_DATASET=llm_arena||BQ_LOCATION=${REGION}||ARENA_LOG_LEVEL=INFO||ARENA_TRADING_MODE=live||ARENA_ALLOW_LIVE_TRADING=true||ARENA_SLEEVE_CAPITAL_KRW=2000000||ARENA_FORCE_SLEEVE_REINIT=false||ARENA_SLEEVE_BOOTSTRAP_FROM_ACCOUNT=false||ARENA_AUTONOMY_WORKING_SET_ENABLED=true||ARENA_AUTONOMY_TOOL_DEFAULT_CANDIDATES_ENABLED=true||ARENA_AUTONOMY_OPPORTUNITY_CONTEXT_ENABLED=true||GOOGLE_GENAI_USE_VERTEXAI=true||GOOGLE_CLOUD_LOCATION=global||ARENA_PUBLIC_DEMO_TENANT=${ARENA_PUBLIC_DEMO_TENANT:-}||ARENA_SHARED_RESEARCH_GEMINI_SOURCE_TENANT=${ARENA_SHARED_RESEARCH_GEMINI_SOURCE_TENANT:-}}"
SCHEDULE_ENV_VARS="${SCHEDULE_ENV_VARS:-ARENA_NASDAQ_CYCLE_TIMES_ET=15:00||ARENA_NASDAQ_CYCLE_TOLERANCE_MINUTES=30||ARENA_NASDAQ_DISABLE_SCHEDULE_GUARD=false||ARENA_KOSPI_CYCLE_TIMES_KST=${ARENA_KOSPI_CYCLE_TIMES_KST:-14:30}||ARENA_KOSPI_CYCLE_TOLERANCE_MINUTES=${ARENA_KOSPI_CYCLE_TOLERANCE_MINUTES:-20}||ARENA_KOSPI_DISABLE_SCHEDULE_GUARD=false}"
KIS_ENV_VARS="${KIS_ENV_VARS:-KIS_TOKEN_CACHE_BACKEND=firestore||KIS_TOKEN_CACHE_COLLECTION=api_tokens||KIS_ENV=real||KIS_TARGET_MARKET=us||KIS_SECRET_NAME=KISAPI||KIS_SECRET_VERSION=latest||KIS_ACCOUNT_KEY_SUFFIX=CO||KIS_CONFIRM_FILLS=true||KIS_CONFIRM_TIMEOUT_SECONDS=25||KIS_CONFIRM_POLL_SECONDS=2.0||ARENA_USD_KRW_FX_MARKET_DIV_CODE=${ARENA_USD_KRW_FX_MARKET_DIV_CODE:-X}||ARENA_USD_KRW_FX_SYMBOL=${ARENA_USD_KRW_FX_SYMBOL:-USDKRW}}"
PREP_ENV_VARS="${PREP_ENV_VARS:-ARENA_UNIVERSE_RUN_TOP_N=1000||ARENA_UNIVERSE_PER_EXCHANGE_CAP=500||ARENA_US_UNIVERSE_PER_EXCHANGE_CAP=385||ARENA_OPPORTUNITY_RANKER_MAX_SCORING_ROWS=1000||ARENA_FORECAST_RANKER_TOP_PER_BUCKET=10||ARENA_FORECAST_MAX_TICKERS=80||ARENA_FORECAST_RANKER_MAX_AGE_HOURS=336||ARENA_US_QUOTE_EXCHANGES=NAS,NYS||ARENA_FORECAST_MODE=all||ARENA_FORECAST_BASE_MODELS=neural}"
AGENT_ENV_VARS="${AGENT_ENV_VARS:-ARENA_LLM_TIMEOUT_SECONDS=1500||ARENA_LLM_TIMEOUT_TRADING_SECONDS=3000||ARENA_ADK_RETRY_MAX=5||ARENA_ADK_RETRY_BACKOFF_SECONDS=10.0||ADK_ENABLE_JSON_SCHEMA_FOR_FUNC_DECL=true}"
OPTIONAL_ENV_VARS=""
if [[ -n "${FRED_API_KEY:-}" ]]; then
  OPTIONAL_ENV_VARS="${OPTIONAL_ENV_VARS}${OPTIONAL_ENV_VARS:+${ENV_PAIR_DELIM}}FRED_API_KEY=${FRED_API_KEY}"
fi
if [[ -n "${ECOS_API_KEY:-}" ]]; then
  OPTIONAL_ENV_VARS="${OPTIONAL_ENV_VARS}${OPTIONAL_ENV_VARS:+${ENV_PAIR_DELIM}}ECOS_API_KEY=${ECOS_API_KEY}"
fi
if [[ -n "${SEC_USER_AGENT:-}" ]]; then
  OPTIONAL_ENV_VARS="${OPTIONAL_ENV_VARS}${OPTIONAL_ENV_VARS:+${ENV_PAIR_DELIM}}SEC_USER_AGENT=${SEC_USER_AGENT}"
elif [[ -n "${ARENA_OPERATOR_EMAILS:-}" ]]; then
  _SEC_CONTACT="${ARENA_OPERATOR_EMAILS%%,*}"
  OPTIONAL_ENV_VARS="${OPTIONAL_ENV_VARS}${OPTIONAL_ENV_VARS:+${ENV_PAIR_DELIM}}SEC_USER_AGENT=LLM Arena ${_SEC_CONTACT}"
fi
...
AGENT_TASK_TIMEOUT="${AGENT_TASK_TIMEOUT:-7200s}"
...
_set_delimited_env_var() {
  ...
  for entry in "${_ENV_ENTRIES[@]}"; do
    ...
    if [[ -n "${result}" ]]; then
      result="${result}${ENV_PAIR_DELIM}${entry}"
    else
      result="${entry}"
    fi
  done
  if [[ "${found}" != "true" ]]; then
    if [[ -n "${result}" ]]; then
      result="${result}${ENV_PAIR_DELIM}${key}=${value}"
    else
      result="${key}=${value}"
    fi
  fi
...
_join_env_vars() {
  ...
  for chunk in "$@"; do
    [[ -z "${chunk}" ]] && continue
    if [[ -n "${result}" ]]; then
      result="${result}${ENV_PAIR_DELIM}${chunk}"
    else
      result="${chunk}"
    fi
  done
...
_market_env_vars() {
  ...
  if [[ -n "${extra_envs}" ]]; then
    env_vars="${env_vars}${ENV_PAIR_DELIM}${extra_envs}"
  fi
  printf '%s' "${env_vars}"
}
...
_deploy_job() {
  ...
  local deploy_cmd=(
    ...
    --set-env-vars "^${ENV_PAIR_DELIM}^${env_vars}"
  )
...
if [[ -n "${RUN_ENV_VARS}" ]]; then
  PREP_JOB_ENV_VARS="${RUN_ENV_VARS}"
  AGENT_JOB_ENV_VARS="${RUN_ENV_VARS}"
  PIPELINE_JOB_ENV_VARS="${RUN_ENV_VARS}"
else
  PREP_JOB_ENV_VARS="$(_join_env_vars "${BASE_ENV_VARS}" "${SCHEDULE_ENV_VARS}" "${KIS_ENV_VARS}" "${PREP_ENV_VARS}" "${OPTIONAL_ENV_VARS}")"
  AGENT_JOB_ENV_VARS="$(_join_env_vars "${BASE_ENV_VARS}" "${SCHEDULE_ENV_VARS}" "${KIS_ENV_VARS}" "${AGENT_ENV_VARS}" "${OPTIONAL_ENV_VARS}")"
  PIPELINE_JOB_ENV_VARS="$(_join_env_vars "${BASE_ENV_VARS}" "${SCHEDULE_ENV_VARS}" "${KIS_ENV_VARS}" "${PREP_ENV_VARS}" "${AGENT_ENV_VARS}" "${OPTIONAL_ENV_VARS}")"
fi
...
if [[ "${DUAL_MARKET,,}" == "true" ]]; then
  if [[ "${A_SPLIT_JOBS,,}" == "true" ]]; then
    ...
    _deploy_job "${PREP_KR_JOB}" "${PREP_RUN_COMMAND}" "${PREP_RUN_ARGS},--market,kospi,--stage,fast,--dispatch-job,${AGENT_KR_JOB}" "${PREP_CPU}" "${PREP_MEMORY}" "${PREP_TASK_TIMEOUT}" "${PREP_TASKS}" "${PREP_PARALLELISM}" "$(_market_env_vars kospi "${PREP_JOB_ENV_VARS}")"
    _deploy_job "${AGENT_US_JOB}" "${AGENT_RUN_COMMAND}" "${AGENT_RUN_ARGS},--market,us" "${AGENT_CPU}" "${AGENT_MEMORY}" "${AGENT_TASK_TIMEOUT}" "${AGENT_TASKS}" "${AGENT_PARALLELISM}" "$(_market_env_vars us "${AGENT_JOB_ENV_VARS}" "ARENA_BATCH_PARALLEL=${AGENT_BATCH_PARALLEL}${ENV_PAIR_DELIM}ARENA_TENANT_LEASE_ENABLED=true")"
    _deploy_job "${AGENT_KR_JOB}" "${AGENT_RUN_COMMAND}" "${AGENT_RUN_ARGS},--market,kospi" "${AGENT_CPU}" "${AGENT_MEMORY}" "${AGENT_TASK_TIMEOUT}" "${AGENT_TASKS}" "${AGENT_PARALLELISM}" "$(_market_env_vars kospi "${AGENT_JOB_ENV_VARS}" "ARENA_BATCH_PARALLEL=${AGENT_BATCH_PARALLEL}${ENV_PAIR_DELIM}ARENA_TENANT_LEASE_ENABLED=true")"
    ...
    _deploy_job "${US_JOB}" "${RUN_COMMAND}" "${US_ARGS}" "${CPU}" "${MEMORY}" "${TASK_TIMEOUT}" "1" "1" "$(_market_env_vars us "${PIPELINE_JOB_ENV_VARS}")"
    _deploy_job "${KR_JOB}" "${RUN_COMMAND}" "${KR_ARGS}" "${CPU}" "${MEMORY}" "${TASK_TIMEOUT}" "1" "1" "$(_market_env_vars kospi "${PIPELINE_JOB_ENV_VARS}")"
  ...
  _cleanup_dual_market_artifacts
  _deploy_job "${JOB_NAME}" "${RUN_COMMAND}" "${RUN_ARGS}" "${CPU}" "${MEMORY}" "${TASK_TIMEOUT}" "1" "1" "$(_market_env_vars "${DEFAULT_TARGET_MARKET}" "${PIPELINE_JOB_ENV_VARS}")"
