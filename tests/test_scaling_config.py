...
def test_scaled_universe_and_ranker_defaults(monkeypatch) -> None:
    ...
    monkeypatch.delenv("ARENA_US_UNIVERSE_PER_EXCHANGE_CAP", raising=False)
    ...
    assert settings.us_universe_per_exchange_cap == 385
...
def test_cloud_run_deploy_defaults_match_scaled_universe() -> None:
    ...
    assert "ARENA_US_UNIVERSE_PER_EXCHANGE_CAP=385" in script
