    ...
    def recent_trade_history(
        ...
        *,
        ...
        filters = [
            ...
            "er.status IN UNNEST(@statuses)",
            "er.created_at >= TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL @days DAY)",
        ...
        FROM `{self.session.dataset_fqn}.execution_reports` er
        LEFT JOIN `{self.session.dataset_fqn}.agent_order_intents` oi
