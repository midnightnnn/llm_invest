...
class MemoryBQStore:
    ...
    def watch_item_by_key(
        ...
        *,
        ...
        rows = self.session.fetch_rows(
            ...
            FROM `{self.session.dataset_fqn}.agent_watch_items`
            WHERE tenant_id = @tenant_id
    ...
    def upsert_watch_item(self, row: dict[str, Any], *, tenant_id: str | None = None) -> None:
        ...
        delete_sql = f"""
        DELETE FROM `{self.session.dataset_fqn}.agent_watch_items`
        WHERE tenant_id = @tenant_id
          AND watch_key = @watch_key
        """
        self.session.execute(delete_sql, {"tenant_id": tenant, "watch_key": watch_key})
        insert_sql = f"""
        INSERT INTO `{self.session.dataset_fqn}.agent_watch_items`
        ({', '.join(built.keys())})
        VALUES ({', '.join(f'@{key}' for key in built.keys())})
        """
        self.session.execute(insert_sql, built)
