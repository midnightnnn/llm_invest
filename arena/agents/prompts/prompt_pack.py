...
class PromptPack:
    ...
    def file_core_prompt() -> str:
        prompt_path = Path(__file__).resolve().parent / "core_prompt.txt"
        try:
            text = prompt_path.read_text(encoding="utf-8").strip()
        except Exception as exc:
            raise RuntimeError(f"Failed to load core prompt: path={prompt_path} err={exc}") from exc
        if "{agent_id}" not in text:
            raise RuntimeError(f"Invalid core prompt: missing '{{agent_id}}' placeholder at {prompt_path}")
        return text
    ...
    def file_user_prompt_default() -> str:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load default user prompt: path={prompt_path} err={exc}") from exc
